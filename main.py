...
def comment(video_id):
    if 'email' in session:
        ...
        cursor.execute("SELECT filename,username,category FROM videos WHERE video_id = ?", (video_id,))
        users = cursor.fetchall()
    ...
    # Render the template and pass the necessary data
        return render_template('head.html', content='home.html',email=session['email'],comment=comment,users=users)
    return redirect(url_for('login'))
...
def comment_show():
    if 'email' in session:
        ...
        cursor.execute("SELECT filename,username,category FROM videos WHERE reg_id = ?", (reg_id,))
        users = cursor.fetchall()
        cursor.execute("SELECT email FROM register_user WHERE reg_id = ?", (reg_id,))
        reg = cursor.fetchall()
        cursor.execute("SELECT comment_text FROM comment WHERE reg_id = ?", (reg_id,))
    ...
    # Render the template and pass the necessary data
        return render_template('head.html', content='comment_show.html',email=session['email'],users=users,reg=reg,comment=comments)
    return redirect(url_for('login'))
